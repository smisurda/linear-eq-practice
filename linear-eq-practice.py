		...
		if len(number) > 0:
			...
			try: 
				if (int(number) * st.session_state.a + st.session_state.b) == st.session_state.c:
					...
					st.write(f"<span style=\"color:red\">⚠️ Sorry, {number} is not quite right. \n You should {operation} {st.session_state.b} {'to' if operation == 'add' else 'from'} both sides, and then divide {st.session_state.a} from both sides.<span>",unsafe_allow_html=True)
			except ValueError:
